...
func TestTaskGetRowAllDocumentedColumns(t *testing.T) {
	...
	tests := []struct {
		...
		{"CREATED_ON", createdOn.Format(common.TableTimeFormat)},
		{"updated_on", updatedOn.Format(common.TableTimeFormat)},
		{"updated on", updatedOn.Format(common.TableTimeFormat)},
		{"resolved_on", resolvedOn.Format(common.TableTimeFormat)},
		{"resolved on", resolvedOn.Format(common.TableTimeFormat)},
		{"state", "RESOLVED"},
