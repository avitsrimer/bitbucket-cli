...
func (activity Activity) GetRow(headers []string) []string {
	...
	for _, header := range headers {
		...
		case "description":
			row = append(row, activity.updateField(func(update *Update) string { return update.Description }))
		case "state":
			row = append(row, state)
		case "author":
			row = append(row, activity.updateField(func(update *Update) string { return update.Author.Name }))
		case "closed by":
			row = append(row, activity.updateField(func(update *Update) string { return update.ClosedBy.Name }))
		case "reason":
			row = append(row, activity.updateField(func(update *Update) string { return update.Reason }))
		case "user":
			row = append(row, actor.Name)
		case "destination":
			row = append(row, activity.updateField(func(update *Update) string {
		...
		case "created on", "created_on", "created-on", "created":
			row = append(row, activity.updateField(func(update *Update) string { return update.CreatedOn.Format("2006-01-02 15:04:05") }))
...
func (activity Activity) updateField(get func(*Update) string) string {
