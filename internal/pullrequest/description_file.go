...
// descriptionFile whose content is empty (after trimming) is rejected: --description alone
// legitimately clears the description (it is an optional field), but a --description-file
// pointing at an empty file is a foot-gun, not a deliberate choice, so it fails instead of
// silently producing the same result.
