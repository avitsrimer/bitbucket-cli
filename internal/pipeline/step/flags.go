...
// pipelineFlagValue reads cmd's own --pipeline value directly (rather than a package-level
// destination), mirroring pipeline/list.go's queryFlagValue: every RunE function in this package
// reads its flags off the passed cmd, so behavior is identical whether cmd is the real command or
// a standalone test double carrying its own --pipeline flag.
