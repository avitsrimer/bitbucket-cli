...
//     not happen.
//   - Any read that comes back with no input at all -- an EOF (e.g. /dev/null on stdin: it passes
//     the character-device check below since /dev/null IS a char device, but the read still
//     returns nothing) or any other read error (e.g. losing the controlling terminal mid-prompt) --
//     is treated as an error, not Confirm's silent decline: nobody answered, so nothing may look
//     like a handled cancel. A real answer with no trailing newline (e.g. strings.NewReader("y"))
//     still returns that answer, not an error: only a wholly empty read counts.
//
// --dry-run still short-circuits first: reporting proceed unconditionally here lets the caller's
// own WhatIf gate -- which always fires first at ConfirmInteractive's one call site -- remain the
// single place that reports what a dry run would have done. This is a fail-open convenience for
// that ordering, not a defense: the branch below is currently unreachable from mergeProcess
// because WhatIfPayload already returns before ConfirmInteractive is ever called under --dry-run,
// but ConfirmInteractive does not rely on that caller ordering to stay correct on its own.
//
...
// error below -- a test's cmd.SetIn stand-in bypasses that specific check, but an empty read from
// that stand-in (whether it errors with io.EOF or something else) still hits the empty-read rule.
