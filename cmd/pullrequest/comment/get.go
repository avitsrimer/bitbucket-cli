...
func getProcess(cmd *cobra.Command, args []string) (err error) {
	...
	lgr.Printf("[DEBUG] displaying pullrequest comment %s", args[0])
	if !common.WhatIf(cmd, "Showing pullrequest comment "+args[0]) {
		return nil
	}
	...
	if err != nil {
		return errors.Join(errors.Errorf("Failed to get pullrequest comment %s", args[0]), err)
	}
